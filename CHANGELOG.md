...
## ✨ 新機能
- npx vibehawk update で workflow を最新版に更新できるようになる (#373) (d4ad213)

## 📖 ドキュメント
- README のアップデート手順を vibehawk update に修正する (#374) (708bb68)

## v0.4.0 - 2026-06-20

## ✨ 新機能
- npx vibehawk setup がロゴ画像を Finder で自動的に開いて設定しやすくなる (#365) (a2009ea)

## 🐛 バグ修正
- vibehawk ロゴが正方形アイコン枠でも角まで隙間なく表示されるようになる (#366) (332c0b4)
- npx vibehawk setup が VIBEHAWK_PRIVATE_KEY の Secrets 登録ページも案内するようになる (#364) (d4aad5f)
- setup ウィザードで OAuth トークン貼り付けプロンプトが表示されるようになる (#363) (557681b)

## 📖 ドキュメント
- setup ウィザードのドキュメントを実装の 7 ステップに揃える (#368) (fdf15aa)

## v0.3.1 - 2026-06-14

## 🐛 バグ修正
- npx vibehawk setup が既存の vibehawk-for-<owner> App を再利用してセットアップを完了できるようになる (#357) (d2400e9)
...
## 🐛 バグ修正
- release.yml の workflow_dispatch が YAML 不正で認識されない不具合を直す (#340) (60961c9)

## 🔧 その他
- リリース PR 内で version bump が自動実行されるようになる（版上げ係の明文化） (#342) (9aab291)
...
- 💥 **#172 fix: `.coderabbit.yaml` フォールバック読み込みを撤廃**
  - `vibehawk_config` step が読み込む設定ソースを `.vibehawk.yaml` 単独受付に絞った。
  - `source_label` の値域も `vibehawk` / `default` の 2 値に縮退（旧 `coderabbit` 値は撤廃）。
  ...
  - **背景**: v0.1.0（Issue #10）で実装した CodeRabbit 互換読み込みは CEO 確定方針（2026-05-18）「互換機能は不要、削除すべき」に基づき撤廃。CodeRabbit と vibehawk は別プロダクト。vibehawk を利用するなら `.vibehawk.yaml` を配置するのが筋。
...
vibehawk MVP リリース。
利用者の Claude Pro / Max サブスクリプション枠の内側で完結し、追加課金ゼロで CodeRabbit 同等のレビュー体験を提供する OSS。

#### 機能カタログ

- ✨ **PR 全体サマリ + インクリメンタルレビュー** (#8): push のたびにサマリコメントが edit で最新化され、コメントが増殖しない。force push / rebase 検出で完全再レビューに自動切替。
- ✨ **severity 5 段階付き inline comment + auto_resolve + sticky review state** (#9): 🔴 Critical / 🟠 Major / 🟡 Minor / 🔵 Trivial / ⚪ Info の絵文字付き inline 指摘。push で直った旧指摘は自身投稿のみ resolved 化。未解決数で都度 approve / request_changes を発行。
- ✨ **`.vibehawk.yaml` 設定 + CodeRabbit 互換** (#10): `path_filters` / `path_instructions` / `size_limits`（段階的劣化）/ `language` (ja/en) を制御。`.coderabbit.yaml` 互換読み込みで既存利用者がそのまま移行可能（※ Unreleased の破壊的変更で撤廃予定）。
- ✨ **`@mention` チャット応答** (#11): PR / Issue で `@vibehawk` メンションするとスレッド全体を読んで多ターン会話。状態は GitHub のスレッド自体が保持し、専用 DB を持たない（5 大方針 4）。

#### 利用者導入

経路 2 必須化により、以下の流れで導入する。
CLI は GitHub Secrets を一切 touch しない（利用者が GitHub Settings UI で手動登録、Issue #72 / #74 確定）。
...
- 🔒 **CodeRabbit 事件 (2025-08) の構造的回避**: 利用者ごとに独立 App `vibehawk-for-<owner>` を採用。Private Key 漏洩の影響範囲は利用者本人のリポジトリ群に限定（集中 SaaS App とは構造的に異なる）。
...
- **Issue #22 妥協の撤回**: 「CEO の GitHub App Private Key を全利用者に配布する設計」と「利用者ごとに独立した App を利用者本人が作成・運用する設計」を区別していなかった当初判定を見直し、後者は Value 1 と整合することが判明。経路 1（`secrets.GITHUB_TOKEN` + `github-actions[bot]`）の妥協を撤回した。経路 2 必須化（CEO 判断 2026-05-09、Issue #72）。

### CISO 承認

経路 2 必須化版で CISO 条件付き承認発行（Issue #62）。
致命的脆弱性なし。新規発生リスク 6 件のうち 3 件「許容」、3 件「Minor 要対策」（Issue #81 で対応済）。
