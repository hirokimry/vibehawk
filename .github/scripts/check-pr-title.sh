...
#
# 判定基準:
#   PR タイトルが「<絵文字> <CC prefix>: <subject>」形式に従うかを正規表現で機械判定。
#   CC prefix 11 種 (feat/fix/perf/refactor/style/docs/test/ci/chore/build/revert)。
...
# 機能: CC prefix + コロンを検出（絵文字は任意の Unicode を許容するため正規表現は緩く）
if printf '%s' "$PR_TITLE" | grep -qE '^(.+ )?(feat|fix|perf|refactor|style|docs|test|ci|chore|build|revert)(\(.+\))?:[[:space:]]'; then
  status="passed"
