#!/usr/bin/env bash
# tests/test_release_run_tests.sh
#
# scripts/ci/release/run-tests.sh の単体テスト（Issue #179）。
#
# 検証内容:
...
SCRIPT="${REPO_ROOT}/scripts/ci/release/run-tests.sh"
...
echo "=== scripts/ci/release/run-tests.sh 単体テスト ==="
