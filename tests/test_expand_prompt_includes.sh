...
# 一時作業ディレクトリは repo root 配下に作る（include 解決基点 = スクリプト位置由来の
# vibehawk リポジトリ root のため、Issue #346）。
...
echo "Case 10: 外部リポジトリ相当の CWD から実行しても include 基点はスクリプト位置由来（Issue #346）"
# 外部リポジトリでは runtime checkout 内のスクリプトが対象リポジトリ root を CWD として実行される。
# CWD（= 対象リポジトリ）に include 先が無くても、vibehawk リポジトリ実体の
# templates/review-prompt.md が解決されることを検証する。
EXTERNAL_CWD="$(mktemp -d "${TMPDIR:-/tmp}/vibehawk-external-repo.XXXXXX")"
cleanup_all() {
  cleanup
  rm -rf "$EXTERNAL_CWD" || true
}
trap cleanup_all EXIT
src10="${WORKDIR}/src10.md"
printf 'HEAD\n<!-- vibehawk:include templates/review-prompt.md -->\nTAIL\n' > "$src10"
out10="$(cd "$EXTERNAL_CWD" && "$SCRIPT" "$src10" 2>/dev/null)"
expected10_head="$(head -1 "${REPO_ROOT}/templates/review-prompt.md")"
if printf '%s\n' "$out10" | grep -qF "$expected10_head" && ! printf '%s\n' "$out10" | grep -qF 'vibehawk:include'; then
  pass "Case 10"
else
  fail "Case 10: 外部 CWD からの include 展開に失敗"
fi
