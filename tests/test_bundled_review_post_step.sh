...
run_step() {
  # 共通環境変数: REPO / PR_NUMBER / GH_TOKEN / RUNNER_TEMP / STRUCTURED_OUTPUT
  # gh スタブを PATH 先頭に追加して実 gh より優先させる
  # Issue #164 fix で `$GITHUB_WORKSPACE` 経由（ファイル事前生成）から `$STRUCTURED_OUTPUT` env
  # 経由（step 内で `${RUNNER_TEMP}` に書き出し）に切り替わった。各ケースは `STRUCTURED_OUTPUT`
  # 環境変数を設定してから run_step を呼ぶ。
...
echo ""
echo "--- ケース 4: REQUEST_CHANGES event の正常 JSON → POST 1 回（testing-2 必須維持） ---"
reset_logs
STRUCTURED_OUTPUT='{"event":"REQUEST_CHANGES","body":"<!-- vibehawk:summary -->\n<!-- vibehawk:sha=feedbeef -->\nテストサマリ","commit_id":"feedbeef","comments":[{"path":"src/foo.ts","line":42,"side":"RIGHT","body":"🟠 **Major**: test"}]}'
export STRUCTURED_OUTPUT
...
STRUCTURED_OUTPUT='{"event":"REQUEST_CHANGES","body":"summary","commit_id":"deadbeef","comments":[{"path":"src/foo.ts","line":1,"body":"line1\nline2\nline3"}]}'
export STRUCTURED_OUTPUT
...
STRUCTURED_OUTPUT='{"event":"APPROVE","body":"quote test: \"hello world\" and \"foo\"","commit_id":"deadbeef","comments":[]}'
export STRUCTURED_OUTPUT
...
STRUCTURED_OUTPUT='{"event":"REQUEST_CHANGES","body":"🚨 vibehawk: 重要な指摘あり / Critical 1 件、Major 2 件","commit_id":"cafebabe","comments":[{"path":"src/日本語.ts","line":1,"body":"🟠 **Major**: 日本語による指摘内容、改行や絵文字 🎉 も含む"}]}'
export STRUCTURED_OUTPUT
