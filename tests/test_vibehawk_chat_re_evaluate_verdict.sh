#!/usr/bin/env bash
# scripts/ci/vibehawk-chat/re-evaluate-verdict.sh の単体テスト（Issue #290、epic #289 子1）。
#
# 検証対象: `@vibehawk review` 差分なし経路での verdict 再評価軽量パス。
#   - vibehawk 自身スレッドの未解決件数のみで APPROVE / REQUEST_CHANGES を決める
#   - 自 Bot スレッドが 0 件なら skip（管轄外）
#   - 直近 review state と一致なら POST skip（冪等）
#   - GITHUB_OUTPUT への decided_event / unresolved_count 出力（post-recheck-notice 連携の根拠）
# gh をスタブして graphql（スレッド+author）/ reviews 一覧 / reviews POST を制御する。
# Issue #287（closed PR #288）の test_vibehawk_review_re_evaluate_verdict.sh を chat 経路へ移設。
...
SCRIPT="${REPO_ROOT}/scripts/ci/vibehawk-chat/re-evaluate-verdict.sh"

echo "=== scripts/ci/vibehawk-chat/re-evaluate-verdict.sh 単体テスト ==="
