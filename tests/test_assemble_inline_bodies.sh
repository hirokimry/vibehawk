...
# --- Case: start_line / start_side を持つ comment でキーと値が pass-through される ---
  ...
  "comments": [
    {
      ...
      "title": "タイトルC",
      "description": "説明C",
      "ai_prompt": "src/c.ts の 8-10 行目を直す"
    }
  ]
}
JSON
...
if [[ "$keys_start" == '["body","line","path","side","start_line","start_side"]' ]]; then
  pass "start_line / start_side が存在する場合にキーが pass-through される"
else
...
if [[ "$start_line_val" == "8" && "$start_side_val" == "RIGHT" ]]; then
  pass "start_line / start_side の値が正しく保持される（8 / RIGHT）"
else
  fail "start_line / start_side の値が変わった（${start_line_val} / ${start_side_val}）"
fi
