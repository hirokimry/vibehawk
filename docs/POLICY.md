...
- **secrets 上書き**: `npx vibehawk setup-token` が利用者の同意（`[Y/n]` プロンプト）の上で既存 `CLAUDE_CODE_OAUTH_TOKEN` を上書きしても、その結果生じる工程停止・他 workflow への影響について vibehawk 開発者は責任を負わない
...
- 本 CLI が変更した GitHub Secrets / App / workflow ファイルの内容を利用者自身で確認・運用すること
