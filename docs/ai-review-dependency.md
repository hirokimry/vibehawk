...
> [!NOTE]
> 本ドキュメントは vibecorp の AI レビュー機構（**CodeRabbit** と **claude-code-action** の 2 経路並走）の責任分担・運用挙動・解約時の挙動を整理する。
> 対象読者: vibecorp をセットアップする開発者。

## 4 契約 × ツール 責任分担表

vibecorp が利用者に提供する AI レビューの 4 契約を、各ツールがどう実現するかを下表に示す。
...
CodeRabbit と claude-action が両方有効な場合、マージ条件は GitHub のネイティブ挙動で決まる。
...
`install.sh` は各 `enabled` フラグを見て以下のように動作する。

- `coderabbit.enabled: false` → `.coderabbit.yaml` を生成しない。既存ファイルは触らない。
- `claude_action.enabled: false` → `.github/workflows/ai-review.yml` と `REVIEW.md` を配布しない。管理下の既存ファイルは削除する（#468 参照）。
...
`skip_paths` は単一の入力源として機能し、`REVIEW.md` の skip rules セクションと `.coderabbit.yaml` の `path_filters` の両方に自動反映される（#465 参照）。
...
claude-code-action 側の挙動は `REVIEW.md`（vibecorp 配布のプロンプト）と `templates/.github/workflows/ai-review.yml`（ジョブ定義）に記述する。
