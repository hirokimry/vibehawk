...
v0.2.3 以降、外部リポジトリに配布された workflow（`vibehawk-review.yml` / `vibehawk-chat.yml`）は、実行時に `hirokimry/vibehawk` をリリースタグ pin 付きで `.vibehawk-runtime/` に checkout し、取得したスクリプト群を 3 secrets（`VIBEHAWK_APP_ID` / `VIBEHAWK_PRIVATE_KEY` / `CLAUDE_CODE_OAUTH_TOKEN`）を env に持つ step 文脈で実行する。これは「利用者 CI が vibehawk リポジトリのタグが指すコードを信頼する」新しい信頼境界である。

`vibehawk-review-skip-mark.yml`（Issue #350）も同じランタイム取得方式（リリースタグ pin 付き 2nd checkout + `persist-credentials: false`）で配布されるが、3 secrets は一切使用せず `GITHUB_TOKEN` のみで実行される。信頼境界は review/chat より狭く、3 secrets の露出起点がそもそも存在しない。
...
- **タグ可変性**: git タグは force-update 可能。本リポジトリは `v*` タグの update / delete を禁止する tag ruleset（creation は release-tag.yml のため許可、bypass actors なし）を運用前提とする。ruleset 自体は admin 権限で編集可能なため、これは緩和であって根治ではない（恒久対応の commit SHA pin 化は別 Issue で扱う）。タグ削除・force-update・リポジトリ rename を行うと既配布の全外部リポジトリのレビューが停止または改変される。
