...
### 配布 workflow のランタイム取得の信頼境界（Issue #346）

v0.2.3 以降、外部リポジトリに配布された workflow（`vibehawk-review.yml` / `vibehawk-chat.yml`）は、実行時に `hirokimry/vibehawk` を **commit SHA pin 付き**で `.vibehawk-runtime/` に checkout し、取得したスクリプト群を 3 secrets（`VIBEHAWK_APP_ID` / `VIBEHAWK_PRIVATE_KEY` / `CLAUDE_CODE_OAUTH_TOKEN`）を env に持つ step 文脈で実行する。これは「利用者 CI が vibehawk リポジトリの **特定 commit** が指すコードを信頼する」新しい信頼境界である（Issue #347 で配布時にリリースタグ → commit SHA を解決し焼き込む方式に変更）。

`vibehawk-review-skip-mark.yml`（Issue #350）も同じランタイム取得方式（commit SHA pin 付き 2nd checkout + `persist-credentials: false`）で配布されるが、3 secrets は一切使用せず `GITHUB_TOKEN` のみで実行される。信頼境界は review/chat より狭く、3 secrets の露出起点がそもそも存在しない。
...
| pin | `ref` は配布時に `cli/install.js` がリリースタグ `v<X.Y.Z>` を解決した **commit SHA**（immutable、Issue #347）。`<commit SHA>  # v<X.Y.Z>`（タグ名コメント併記）として焼き込む。利用者が検収した npm バージョンに対応する特定 commit が実行され、タグ差し替えの影響を受けない。解決は upstream `hirokimry/vibehawk` から行い、解決失敗（オフライン / API 障害 / タグ不在 / 不正 SHA）時は fail-fast で配布を中止する（不正な ref のまま配布しない） |

#### 残余リスク（CISO 記録）

- **タグ可変性（commit SHA pin で根治、Issue #347）**: git タグは force-update 可能（mutable）だが、配布 workflow の `ref` は配布時に解決した immutable な commit SHA を焼き込むため、タグ差し替えによる既配布の全外部リポジトリへのコード波及を**構造的に受けない**。`v*` タグの update / delete を禁止する tag ruleset（creation は release-tag.yml のため許可、bypass actors なし）は admin 権限で編集可能であり、commit SHA pin を補完する多層防御の**緩和措置**（CEO がリポジトリ設定で実施）と位置づける。SHA pin 後の残余リスクは「配布時点の `cli/install.js` 実行環境（利用者ローカル）の侵害」のみに縮小する。
- **ランタイム解決 guard の評価位置**: 自リポジトリ / 外部リポジトリの判定（`hashFiles('scripts/ci/vibehawk-*/check-secrets.sh')`）は PR head checkout（= PR 著者支配下）の内容で評価される。same-repo PR が同パスのファイルを追加すると手元スクリプト実行に倒れるが、`pull_request` トリガーでは workflow 定義自体が PR head から読まれるため、PR 著者は guard を突くまでもなく workflow を直接書き換えられる立場にあり、fork PR には secrets が供給されない。したがって本 guard による**新規の権限獲得・secrets 露出の増分はゼロ**（CISO 判定: コード変更不要、残余リスクとして記録）。
