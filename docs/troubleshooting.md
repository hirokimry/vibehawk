...
## bot アイコンが花束のままで vibehawk ロゴにならない

PR コメント・レビュー・ラベル付与など bot の露出箇所で、アイコンがデフォルトの花束のまま vibehawk ロゴ（🦅）にならない場合。
...
2. `Display information` の現在のロゴ（花束アイコン）をクリックする
3. 同梱の vibehawk ロゴ画像をドラッグ&ドロップしてアップロードする
   - 画像の場所: 配布物同梱の `assets/vibehawk-logo.png`（PNG / 200×200px / 1MB 未満）
