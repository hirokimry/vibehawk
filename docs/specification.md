...
10. `<details><summary>📝 Walkthrough</summary>` セクション（Issue #227 / #228、CodeRabbit 互換）。配下に以下を含む:
    ...
    - `## Estimated code review effort`: Claude が schema 必須化された `review_effort: {difficulty, minutes}` フィールドで返す推定レビュー労力を、見出し + `🎯 N (Label) | ⏱️ ~M minutes` 行で展開（Issue #228 / #238、difficulty 1-5 ラベル: Trivial/Easy/Moderate/Complex/Very Complex。値を見出しにしない）
...
| ~~pre-merge checks（タイトル形式・docstring 検証）~~ | **実装済み（Issue #229、sticky body 構造項目 10 参照）** |

### 現状未実装機能（CodeRabbit との差分、Issue #219）

CodeRabbit walkthrough コメントが持つ機能群のうち、vibehawk sticky 経路（Issue #219）が当初 **意図的に再現しないとした 18 機能** を 4 グループに分けて記録する（うち item 5 / item 18 は epic #289 で実装済みのため、現在の未実装は 16 機能）。利用者・開発者は本表で「vibehawk と CodeRabbit の差分」を 1 箇所で把握できる。
...
> [!NOTE]
> vibehawk は既に **`@vibehawk` メンションへの単発応答**（前述「§@mention チャット応答」および §補助機能）を実装済み。本グループの「未実装」はそれを超えた CodeRabbit 互換の拡張機能を指す。境界は各項の「未実装の範囲」列で明示する。
