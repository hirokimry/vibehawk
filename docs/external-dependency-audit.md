...
> 本ドキュメントは vibehawk が依存する外部サービス（GitHub / Anthropic claude-code-action 等）の利用規約・公式ガイドラインに対し、vibehawk の実装・配布物・命名規則が抵触しないことを CLO レビュー観点で確認した監査結果を永続化する Source of Truth である。
...
- `docs/POLICY.md`「法務・コンプライアンスポリシー」
