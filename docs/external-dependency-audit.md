...
- **配置**: `dependencies`（`devDependencies` 不可。`npx vibehawk setup` 実行時にロードされるため配布物に含める必要あり）
- **利用者伝搬**: CLI ツール用途のみ。利用者の workflow 実行時には埋め込まれない（GitHub Actions runner 上で配置される `.github/workflows/vibehawk-*.yml` から `@clack/prompts` を呼ばない）
- **推移的依存ライセンス**: `npm audit`（脆弱性 0 件）+ `npm ls @clack/prompts --all`（4 推移的依存、すべて OSS）で確認済（PR #<番号> 本文に記録）
- **代替検討**: `@inquirer/prompts` / `enquirer` も候補。@clack/prompts は 4KB gzip / TS-native / `group()` API がウィザード向きで採用（業界調査出典は Issue #91 業界調査結果セクション参照）

### 4-3. 監査結論

`@clack/prompts` の依存追加は MIT ライセンス上の通常利用であり、vibehawk MIT と整合。CLO 条件付き承認（4 条件）を満たす形で監査エントリを記録した。
