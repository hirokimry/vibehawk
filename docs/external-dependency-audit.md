...
> [!IMPORTANT]
> 本ドキュメントは vibehawk が依存する外部サービスについて、以下 2 点を永続化する Source of Truth。
> (1) 利用規約・公式ガイドラインに対する規約整合性の CLO 監査結果（Issue #68）。
> (2) 月次のリスク監視履歴（Issue #64）。
...
| `@clack/prompts` | CLI ウィザード UI 依存（MIT、Issue #91） | `https://www.npmjs.com/package/@clack/prompts` |
| Google Gemini（画像生成、"Nano Banana" / Gemini 2.5 Flash Image） | bot アイコン用ロゴ画像の生成（Issue #249） | `https://policies.google.com/terms` |
...
- vibehawk は claude-code-action を **依存ライブラリとして利用** しているのみで、Anthropic との直接契約関係には立たない。
  - 📍 根拠: `docs/POLICY.md` Value 1「利用者の契約だけで、完結させる」
- 利用者の workflow が claude-code-action を呼び出した時点で、利用者と Anthropic の間に契約関係が成立する。
  - 成立する契約: Anthropic Usage Policies / Anthropic Commercial Terms of Service
  - 発行経路: `CLAUDE_CODE_OAUTH_TOKEN` を利用者本人の Anthropic アカウントから発行
...
本免責は claude-code-action のバグ・脆弱性・API 仕様変更・課金挙動を含む。
📍 補足: `docs/POLICY.md`「claude-code-action 依存の免責拡張（Issue #68）」サブセクション

### 3-4. 監査結論

claude-code-action への依存関係は MIT ライセンス上の通常利用であり、利用者と Anthropic の間で契約関係が成立する設計。
vibehawk 開発者は claude-code-action の挙動責任を負わない旨を以下で明示する。

- `docs/POLICY.md` L113
- Issue #68 で追加した拡張サブセクション

## 4. `@clack/prompts`（MIT、CLI ウィザード UI 依存、Issue #91）
...
|------|----|
| ライブラリ | `@clack/prompts` |
| 採用バージョン | `^0.11.0`（v1.x は Node 20+ 要求のため互換性確保で v0 系を採用） |
| ライセンス | MIT（`npm view @clack/prompts@0.11.0 license` 確認済） |
| 提供主体 | Clack Authors（OSS） |
| 機能 | `npx vibehawk setup` 対話型ウィザードの prompt UI（intro / outro / text / select / note / spinner / cancel / isCancel / group） |

### 4-2. CLO 確認結果

- **ライセンス**: MIT — vibehawk MIT と互換、`docs/POLICY.md` L101「MIT 原則許可」と整合
- **配置**: `dependencies`（`devDependencies` 不可）。
  - `npx vibehawk setup` 実行時にロードされるため配布物に含める必要あり。
- **利用者伝搬**: CLI ツール用途のみ。
  - 利用者の workflow 実行時には埋め込まれない。
  - `.github/workflows/vibehawk-*.yml` から `@clack/prompts` を呼ばない。
- **推移的依存ライセンス**: `npm audit`（脆弱性 0 件）+ `npm ls @clack/prompts --all`（4 推移的依存、すべて OSS）で確認済。PR #<番号> 本文に記録。
- **代替検討**: `@inquirer/prompts` / `enquirer` も候補。
  - @clack/prompts は 4KB gzip / TS-native / `group()` API がウィザード向きで採用。
  - 業界調査出典: Issue #91 業界調査結果セクション参照。

### 4-3. 監査結論

`@clack/prompts` の依存追加は MIT ライセンス上の通常利用であり、vibehawk MIT と整合。
CLO 条件付き承認（4 条件）を満たす形で監査エントリを記録した。
...
| `@clack/prompts` 依存（Issue #91） | MIT、CLO 条件付き承認 4 条件を満たす形で導入（推移的依存ライセンス確認 / 監査エントリ追加 / README 必須項目維持 / 月次監視追加） |
...
| `@clack/prompts` | breaking change、ライセンス変更、メンテナンス停止（Issue #91） | install フロー影響 | [release notes](https://github.com/natemoo-re/clack/releases) |
...
| `@clack/prompts` | 変更なし / 変更あり（release tag + 要約） |
...
備考: 本エントリは Issue #64 ポリシー策定時点で baseline として遡及記録したもの。
今後の監査は本ファイルに月次で追記する。
...
## 7. Google Gemini 画像生成（bot アイコン用ロゴ、Issue #249）

### 7-1. 利用内容

- 対象: bot アイコン用ロゴ画像 `assets/vibehawk-logo.png`（PNG / 200×200px）の生成。
- 生成ツール: Google Gemini の画像生成モデル（通称 "Nano Banana" = Gemini 2.5 Flash Image）。
- 配布: npm 配布物（`package.json` の `files` に `assets/` を追加）として MIT で再配布される。
...
| 観点 | 確認内容 | 結論 |
|------|----------|------|
| 生成物の所有権 | Google は生成物の所有権を主張せず、利用者に帰属する（上記条項） | MIT 再配布可 |
| 著作権表記 | `LICENSE` の `Copyright (c) 2026 Hiroki Moriya` と整合（生成主体 = ファウンダー本人） | 整合 |
| ライセンス明示 | `assets/ASSETS-LICENSE.md` で生成 AI 出所・所有権・MIT 適用・商標注記を明示 | 対応済 |
| 第三者意匠の混入 | 抽象的な幾何マーク（三角＋スラッシュの鷹）で特定他社ロゴの複製ではないことを目視確認 | 抵触なし |
| 商標としてのロゴ配布範囲 | POLICY.md の商標条項は App 命名のみ対象でロゴ画像配布が未規定。POLICY 拡張は別 Issue で対応 | 残課題（別 Issue） |
...
|------|----|
| 監査日 | 2026-06-07 |
| 監査主体 | CLO メタレビュー（legal-analyst ×3 全会一致 Critical → CEO 判断 A: 軽減策付きで進行） |
| 残課題 | POLICY.md 商標条項のロゴ画像配布への拡張（別 Issue 起票） |
