...
利用者は GitHub の branch protection で `vibehawk` を required status check に追加するだけで、「AI レビューが OK を出さないと merge できない」merge gate を構築できる。
...
vibehawk は GitHub の `required_approving_review_count`（人間レビュー必須件数）を AI で満たす設計を **意図的に避ける**。
AI が approve を発行できる設計だと「人間レビュー必須要件のバイパス」と見なされる構造的リスクがある。
業界 4 社（Copilot / Gemini / Claude Code Review / Cursor BugBot）の AI レビューも同じ理由で APPROVE 経路を回避している。
...
merge gate の **主軸は status check** であり、`approve` / `request_changes` は補助情報として post されるが merge gating には使わない。
詳細: [`docs/specification.md § status check 仕様`](docs/specification.md)（Issue #138 / #121-C1）。
...
本ステップを実施しないと vibehawk は補助情報（approve / request_changes）を post するのみで merge gate として機能しない。
...
**CLI 自体は Anthropic に通信しない**（localhost のみで完結）。
...
| ドキュメント / リポジトリ | 内容 |
|---|---|
| [`MVV.md`](MVV.md) | プロダクトの根幹方針（編集禁止） |
| [`docs/specification.md`](docs/specification.md) | 機能仕様 / CLI 仕様 / status check 仕様 |
| [`docs/design-philosophy.md`](docs/design-philosophy.md) | 認証経路設計 / 命名統制の設計根拠 |
| [`docs/SECURITY.md`](docs/SECURITY.md) | 認証・認可 / Private Key の CISO Critical 条件 |
| [`docs/POLICY.md`](docs/POLICY.md) | 免責 / PII 取扱い方針 / 商標使用許諾 |
| [`anthropics/claude-code-action`](https://github.com/anthropics/claude-code-action) | vibehawk が呼び出す Anthropic 公式 Action（MIT） |

## 📜 ライセンス / ステータス / 免責

- **ライセンス**: MIT
- **ステータス**: 開発中（Phase 1 基盤構築 + OSS 配布対応）。Issue #7 実行基盤、#22 OSS 配布可能化、#24 `npx vibehawk install` 基盤、#91 `setup` ウィザード（1 コマンド導入）、#121-C1 required status check、#138 status check 主軸 positioning を順次積み上げ
