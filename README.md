...
vibehawk は **branch protection の required status check として動く AI PR レビュアー** であり、**追加課金ゼロの OSS** です。利用者は GitHub の branch protection で `vibehawk` を required status check に追加するだけで「AI レビューが OK を出さないと merge できない」merge gate を構築できます。利用者が既に契約している LLM サブスクリプション枠（Claude Pro / Max 等）の **内側だけ** で動作し、AI レビュー専用 SaaS の月額や LLM API の従量課金を発生させません。

### 設計思想: 人間 review 必須要件をバイパスしない

vibehawk は GitHub の `required_approving_review_count`（人間レビュー必須件数）を AI で満たす設計を **意図的に避けます**。AI が approve を発行できる設計だと「人間レビュー必須要件のバイパス」と見なされる構造的リスクがあり、業界 4 社（Copilot / Gemini / Claude Code Review / Cursor BugBot）の AI レビューも同じ理由で APPROVE 経路を回避しています。

vibehawk は Anthropic が公式ドキュメントで案内している「自前 CI で gate する」設計思想（`claude-code-review` workflow）を OSS としてパッケージ化したものです（Anthropic 提携・公認製品ではありません）。merge gate の主軸は **status check** であり、approve / request_changes は補助情報として post されますが、merge gating には使いません（Issue #138 / #121-C1）。
...
**vibehawk を使う = リポジトリの branch protection に `vibehawk` を required status check として追加すること** がゴールです。これにより「AI レビューの conclusion が `success` でない PR は merge できない」merge gate が成立します。

ゴール達成までの全体フロー:

```text
[ステップ 1-3]                [ステップ 4-6]                  [ステップ 7 ← ゴール]
App / secrets 準備    →   workflow 配置 + 初回 PR 発火   →   branch protection に
（前提準備）              （`vibehawk` check を発火させる）    `vibehawk` を required 登録
                                                              （vibehawk 利用の根幹）
```

ステップ 1-6 は **ステップ 7 を機能させるための前提準備** です。ステップ 7（branch protection への `vibehawk` required 登録）が完了して初めて vibehawk は merge gate として機能します。GitHub の仕様上、`vibehawk` check が一度発火していないと branch protection の検索候補に出ないため、ステップ 7 はステップ 6（初回 PR）の後に実施する手順順序になります。
...
#### 7. branch protection で `vibehawk` を required status check に追加する（vibehawk 利用の根幹）

**このステップが vibehawk 利用の根幹** です。ステップ 1-6 はこのステップを機能させるための前提準備にすぎません。本ステップを実施しないと vibehawk は補助情報（approve / request_changes）を post するのみで merge gate として機能しません。

vibehawk は `POST /repos/X/Y/check-runs` API で `vibehawk` という名前の status check を post します（**merge gate の主軸**）。これに加えて approve / request_changes を **補助情報** として post しますが、GitHub の構造仕様により bot review は branch protection の `required_approving_review_count` に count されないため、merge gating を確実に効かせるには **status check 側で required 指定** する必要があります（CodeRabbit が `["CodeRabbit", "test"]` で行っているのと同じ仕組み、Issue #121-C1 / #138）。

check 名は `vibehawk` 固定で、`vibehawk-for-<owner>` App ではなく workflow のデフォルト `GITHUB_TOKEN`（`permissions.checks: write` 付き）で post されます。利用者は App の再 install なしで status check が動作します（投稿者表示は `github-actions[bot]`、check の `name` は `vibehawk` 固定のため branch protection 設定上の識別性は維持されます）。

設定手順:

1. リポジトリの `Settings → Branches → Branch protection rules` を開く
2. `main`（または保護したいブランチ）の `Edit` をクリック
3. `Require status checks to pass before merging` を ON にする
4. 検索ボックスに `vibehawk` と入力し、出てきた `vibehawk` を required に追加
5. `Save changes`

これにより vibehawk が `failure` conclusion を返した PR は merge ボタンがブロックされます（`success` / `neutral` で解除）。`vibehawk` check が一度も post されていない状態では検索ボックスに候補が出ないため、先にステップ 6 のダミー PR を立てて check を発火させてから本ステップを実施してください。

**この登録を行わない場合**、vibehawk は approve / request_changes を補助情報として post しますが、merge gate として機能しません（bot review は required reviewers に count されないため）。vibehawk を導入したら必ず本ステップまで完了させてください。
...
`npx vibehawk install` 実行時に以下のようなエラーで処理が中断される場合があります（Issue #60 / CEO 判断 B、2026-05-09）:
...
GitHub Apps はグローバルで名前ユニーク制約があり、同名 App が既に存在する場合、GitHub が自動的に連番（例: `vibehawk-for-alice-2`）を付与します。これは [`docs/POLICY.md`](docs/POLICY.md) の「命名は `vibehawk-for-<owner>` 形式に厳密に従うこと」MUST 条件に違反するため、vibehawk CLI は連番命名を検出した時点で処理を中断（exit 1）します。

連番付き App は商標使用許諾の範囲外（`vibehawk-` プレフィックスの未許可派生名）にあたるため、即時削除すべきです。

#### 復旧手順

1. **連番付き App を削除**: 直前の実行で作成された連番付き App（例: `vibehawk-for-alice-2`）を GitHub UI から削除します。
   - エラーメッセージに表示された URL（例: `https://github.com/settings/apps/vibehawk-for-alice-2`）にアクセス
   - 該当 App の **Settings → Delete GitHub App** で削除

2. **既存の `vibehawk-for-<owner>` App を確認**: 同名 App が既に存在することが連番付与の原因です。
...
3. **再実行**: 上記いずれかの対応後、`npx vibehawk install` を再実行すると正しい命名（`vibehawk-for-<owner>` 形式）で App が作成されます。

#### 注意事項

- CLI は連番命名を検出した時点で App credentials の Private Key を [REDACTED] 化してメモリから除去します（CISO Critical 条件）。利用者の手元・画面に Private Key が残存することはありません。
- 連番付き App を放置すると `docs/POLICY.md` の MUST 違反状態が継続し、商標使用許諾の取消条件に該当する可能性があります。発見次第すみやかに削除してください。
