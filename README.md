...
#### 7. （推奨）branch protection で `vibehawk` を required status check に追加（Issue #121-C1）
...
これにより vibehawk が `failure` conclusion を返した PR は merge ボタンがブロックされます（`success` / `neutral` で解除）。`vibehawk` check が一度も post されていない状態では検索ボックスに候補が出ないため、先に 1 度ダミー PR を立てて check を発火させてから登録してください。
